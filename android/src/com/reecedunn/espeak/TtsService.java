...
public class TtsService extends TextToSpeechService {
            ...
            for (Voice voice : mAvailableVoices.values()) {
                switch (voice.match(query)) {
                    case TextToSpeech.LANG_COUNTRY_VAR_AVAILABLE:
                        // #970: Sometimes, onGetDefaultVoiceNameFor is called
                        // with a language, but without a country or variant
                        // (probably due to partial SSML from the system).
                        // If this is the first full variant we've seen for
                        //  this language, cache it for future lookup.
                        if (!mVariantCache.containsKey(language))
    ...
    private Pair<Voice, Integer> getDefaultVoiceFor(String language, String country, String variant) {
        // #970: Sometimes, onGetDefaultVoiceNameFor is called with a
        // language, but without a country or variant (probably due to partial
        // SSML from the system). If this is the case, and we've seen this
        // language before, return the first seen full variant.
        if (
